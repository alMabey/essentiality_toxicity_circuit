...
from scripts.cell_model_tox_only import *
...
def batch_cult_sim(base_params, hPR, xPR, integration_method="BDF", plot=False, rtol=1E-6, atol=1E-9):
    ...
    initial_conditions_solver = sun.cvode.CVODE(
        rhsfn_ss,
    ...
    sol   = initial_conditions_solver.solve(t_eval, np.asarray(y0, dtype=float))
    t = sol.t; y = sol.y                 
    ...
    def rhsfn(t, y, yp):
        yp[:] = BatchCultModel_DC(t, y, hPR, xPR, topology, kin)
    ...
    #Now calculating the vProd and pYield
    vProd = y[2, -1]/t[-1]
    pYield = y[2, -1]/y[1, 0]
    if pYield > 1:
